...
class MagicalStringSolution(object):
    ...
    def fast_MagicalString(self, digit, n):
        '''
        int digit: the number in the string you would like to count
        int n: which position you would like to count to (inclusive)
        ...
        for i in range(2,n):
            l+=[1+i%2]*l[i]
        return l[:n].count(digit) 
        
class MagicalStringTest(unittest.TestCase):
    
    def test1(self):
        s = MagicalStringSolution()
        answer = s.fast_MagicalString(1,6)
        expected = 3
        self.assertEqual(answer, expected)
        
    def test2(self):
        s = MagicalStringSolution()
        answer = s.fast_MagicalString(2,4)
        expected = 2
        self.assertEqual(answer, expected)
        
    def test3(self):
        s = MagicalStringSolution()
        answer = s.fast_MagicalString(1,4)
        expected = 2
        self.assertEqual(answer, expected)
        
    def test4(self):
        s = MagicalStringSolution()
        answer = s.fast_MagicalString(2,6)
        expected = 3
        self.assertEqual(answer, expected)
